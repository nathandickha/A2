...
function getRectanglePoolCopingSideKeyForSnapSide(snapSide) {
...
function clearSpaSplitCoping(poolGroup) {
  if (!poolGroup?.userData) return;
  const splitMeshes = poolGroup.userData.spaSplitCopingMeshes || [];
  splitMeshes.forEach((m) => m?.parent?.remove(m));
  poolGroup.userData.spaSplitCopingMeshes = [];
  const hidden = poolGroup.userData.spaHiddenOriginalCoping || [];
  hidden.forEach((m) => { if (m) m.visible = true; });
  poolGroup.userData.spaHiddenOriginalCoping = [];
}

function updateRectanglePoolCopingForSpa(poolGroup, spaGroup) {
  clearSpaSplitCoping(poolGroup);
  ...
  const snapSide = spaGroup?.userData?.snapSide || null;
  const sideKey = getRectanglePoolCopingSideKeyForSnapSide(snapSide);
  const original = sideKey ? copingSegments[sideKey] : null;
  if (!original?.isMesh || !original.geometry?.parameters) return;
  ...
  const inv = new THREE.Matrix4().copy(poolGroup.matrixWorld).invert();
  const worldToPoolLocal = (x, y, z = 0) => new THREE.Vector3(x, y, z).applyMatrix4(inv);

  let throatMin, throatMax, axis, fixedSize, depthSize;
  const gp = original.geometry.parameters;
  if (sideKey === 'west' || sideKey === 'east') {
    throatMin = worldToPoolLocal(center.x, center.y - halfY - alongPad, center.z).y;
    throatMax = worldToPoolLocal(center.x, center.y + halfY + alongPad, center.z).y;
    axis = 'y';
    fixedSize = gp.width;
    depthSize = gp.depth;
  } else {
    throatMin = worldToPoolLocal(center.x - halfX - alongPad, center.y, center.z).x;
    throatMax = worldToPoolLocal(center.x + halfX + alongPad, center.y, center.z).x;
    axis = 'x';
    fixedSize = gp.height;
    depthSize = gp.depth;
  }
  ...
  const longSize = axis === 'y' ? gp.height : gp.width;
  const sideMin = pos[axis] - longSize * 0.5;
  const sideMax = pos[axis] + longSize * 0.5;

  const cutMin = Math.max(sideMin, throatMin);
  const cutMax = Math.min(sideMax, throatMax);
  if (!(cutMax > cutMin)) return;
  ...
  poolGroup.userData.spaHiddenOriginalCoping = [original];
  poolGroup.userData.spaSplitCopingMeshes = [];

  segments.forEach(([a, b], idx) => {
    const segLen = b - a;
    const geo = axis === 'y'
      ? new THREE.BoxGeometry(fixedSize, segLen, depthSize)
      : new THREE.BoxGeometry(segLen, fixedSize, depthSize);
    if (geo.attributes && geo.attributes.uv && !geo.attributes.uv2) {
      geo.setAttribute('uv2', new THREE.BufferAttribute(geo.attributes.uv.array, 2));
    }
    const mesh = new THREE.Mesh(geo, original.material);
    mesh.castShadow = original.castShadow;
    mesh.receiveShadow = original.receiveShadow;
    mesh.userData = { ...original.userData, isSpaSplitCoping: true, originalSide: sideKey };
    mesh.position.copy(pos);
    mesh.position[axis] = (a + b) * 0.5;
    poolGroup.add(mesh);
    poolGroup.userData.spaSplitCopingMeshes.push(mesh);
  });
}
...
export function updatePoolWaterVoid(poolGroup, spaGroup) {
  ...
  updateRectanglePoolCopingForSpa(poolGroup, spaGroup);
}
